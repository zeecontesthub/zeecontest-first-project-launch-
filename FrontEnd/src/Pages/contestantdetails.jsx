...
const ContestantDetails = () => {
    ...
    for (let i = 0; i < contestants.length; i++) {
      ...
      if (isLast) {
        ...
        const max = Math.floor(
          (remainingVotes / (contestants.length - i)) * 1.5
        );
  ...
  const allContestants = useMemo(() => {
    ...
    return distributeVotes(positionInfo.votersCount, positionInfo.contestants)
      ...
      .map((contestant, index) => ({
        ...
        dailyVotes: Array.from({ length: 7 }, () =>
  ...
  const currentContestant = useMemo(() => {
    if (contestantId) {
      return (
        allContestants.find((c) => c.id === contestantId) || allContestants[0]
      );
    }
  ...
  const dynamicMetrics = useMemo(() => {
    ...
    const nextRankCandidate = allContestants.find(
      (c) => c.rank === currentContestant.rank - 1
    );

    const contestProgress = Math.floor(Math.random() * 40) + 40; // 40-80% progress
    const votesToNextRank = nextRankCandidate
      ? nextRankCandidate.votes - currentContestant.votes + 1
      : 0;
    const votesToWin =
      leadingCandidate && leadingCandidate.id !== currentContestant.id
  ...
  const performanceData = useMemo(() => {
    ...
    return days.map((day, index) => ({
      day,
      votes:
        currentContestant.dailyVotes?.[index] || Math.floor(Math.random() * 5),
    }));
  }, [currentContestant]);
  ...
  const getStrategicRecommendation = () => {
    if (!currentContestant)
      return {
        ...
        description: "Get more votes to climb the leaderboard.",
      };
    ...
    if (rank === 1) {
      return {
        ...
        description: "Keep engaging supporters to secure your lead.",
      };
    } else if (rank <= 3) {
      return {
        action: "Push for the Top!",
        description: `Get ${dynamicMetrics.votesToWin} more votes to take the lead.`,
      };
    } else if (rank <= 5) {
      return {
        action: "Enter Top 3!",
        description: `Get ${
          dynamicMetrics.votesToNextRank
        } more votes to reach rank #${rank - 1}.`,
      };
    } else {
      return {
        action: "Boost Your Campaign!",
        description: "Share your link and mobilize supporters to climb higher.",
      };
    }
  };
                  ...
                  <div className="text-center flex items-center justify-center">
                    ...
                    <div className="ml-2">
                      <div className="text-sm font-medium">
                        {currentContestant.momentum > 0 ? "+" : ""}
                        {currentContestant.momentum}
                      </div>
            ...
            <div className="bg-white rounded-lg p-6 shadow-sm">
              ...
              <div className="space-y-4">
                ...
                <div className="text-center text-sm text-gray-600">
                  Total this week:{" "}
                  {performanceData.reduce((sum, day) => sum + day.votes, 0)}{" "}
                  votes
                </div>
              </div>
            </div>
                ...
                <div className="grid grid-cols-2 gap-4 mt-4">
                  <div className="text-center p-3 bg-gray-50 rounded-lg">
                    <div className="text-lg font-bold text-gray-900">
                      {dynamicMetrics.totalCandidates}
                    </div>
                ...
                {currentContestant.rank > 1 && (
                  <div className="flex justify-between items-center p-3 bg-yellow-50 rounded-lg">
                    ...
                    <span className="font-bold text-yellow-700">
                      +{dynamicMetrics.votesToNextRank} votes
                    </span>
                  </div>
                )}
                {currentContestant.rank !== 1 && (
                  <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg">
                    ...
                    <span className="font-bold text-green-700">
                      +{dynamicMetrics.votesToWin} votes
                    </span>
                  </div>
                )}
                {currentContestant.rank === 1 && (
              ...
              <div className="text-center">
                <div
                  className={`text-3xl font-bold mb-2 ${
                    dynamicMetrics.winningProbability > 70
                      ? "text-green-600"
                      : dynamicMetrics.winningProbability > 40
                      ? "text-yellow-600"
                      : "text-red-600"
                  }`}
                >
            ...
            <div className="bg-white rounded-lg p-6 shadow-sm">
              ...
              <div className="p-4 bg-blue-50 rounded-lg">
                ...
                <div className="text-xs text-blue-700">
                  {strategicRecommendation.description}
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
              {Object.keys(positionData).map((pos) => (
                <button
                  ...
                  onClick={() =>
                    navigate(
                      `/contestant-details/${pos}/${currentContestant.id}`
                    )
                  }
                ...
                >
                  ...
                  <div className="text-xs opacity-75">
                    ({positionData[pos].votersCount} votes)
                  </div>
                </button>
              ))}
            </div>
            ...
            <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
              <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4 shadow-2xl">
                ...
                <p className="text-gray-600 mb-6">
                  ...
                  <strong>{currentContestant.name}</strong> from the{" "}
                  {currentPosition} position? This will delete their{" "}
                  {currentContestant.votes} votes and cannot be undone.
                </p>
